...
/* Import useHistory for React Router v5 support */
...
const HonorManager: React.FC = () => {
  /* Use history hook for v5 instead of useNavigate */
  ...
  const getAttendanceCount = (staffId: string) => {
    ...
    try {
      ...
      let count = 0;
      // Periksa setiap hari di bulan tersebut
      for (let d = 1; d <= 31; d++) {
  ...
  const calculateTotal = (staffId: string) => {
    const count = getAttendanceCount(staffId);
    const rate = rates[staffId] || 0;
    return count * rate;
  };
  ...
  const handleGenerateReceipt = async () => {
    ...
    try {
      ...
      const newMail: Mail = {
        ...
        subject: `Daftar Penerimaan Honor ${activeCategory === 'extra' ? 'Pengajar Ekskul' : 'Tenaga Tukang'} - ${period}`,
        description: `Dokumen rekapitulasi pembayaran honor bulan ${period} berdasarkan absensi otomatis.`,
        category: 'Absensi',
        urgency: UrgencyLevel.LOW,
        status: MailStatus.ARCHIVED,
        fileUrl: pdfDataUri,
        aiSummary: 'Laporan keuangan/honorarium otomatis.'
      };

      await saveMail(newMail);
      alert('Daftar honor berhasil disimpan ke Arsip.');
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4 animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
        <div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tight">Penerimaan Honor</h2>
          <p className="text-slate-500 font-medium">Hitung otomatis honorarium berdasarkan data absensi realtime.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm space-y-6">
            ...
            <div className="bg-indigo-50 p-5 rounded-2xl border border-indigo-100 space-y-3">
               <div className="flex items-center gap-2 text-indigo-700">
                  <Coins size={18} />
                  <span className="text-xs font-black uppercase tracking-widest">Input Tarif</span>
               </div>
               <div className="space-y-3">
        ...
        {/* Receipt Preview */}
        <div className="lg:col-span-8 overflow-x-auto p-4 bg-slate-100 rounded-[2.5rem] border border-slate-200 flex justify-center items-start min-h-[800px]">
           <div 
             ref={receiptRef}
             className="receipt-paper bg-white shadow-2xl p-[15mm] text-black font-['Times_New_Roman'] flex flex-col origin-top scale-[0.7] md:scale-100"
           ...
           >
              ...
              <div className="border-b-[3px] border-double border-black pb-4 mb-8 grid grid-cols-[80px_1fr_80px] items-center">
                 ...
                 <div className="text-center px-4">
                    <h3 className="text-[12pt] uppercase leading-tight">{config.headerLine1}</h3>
                    <h3 className="text-[12pt] font-bold uppercase leading-tight">{config.headerLine2}</h3>
                    <h1 className="text-[16pt] font-black uppercase my-1 leading-none">{config.name}</h1>
                    <p className="text-[9pt] leading-tight">{config.address}</p>
                 </div>
              ...
              <div className="text-center mb-8">
                 ...
                 <p className="text-[12pt] font-bold uppercase mt-1">
                   {activeCategory === 'extra' ? 'TENAGA PENGAJAR EKSTRAKURIKULER' : 'TENAGA TUKANG / PERBAIKAN'}
                 </p>
              ...
              <table className="w-full border-collapse border border-black text-[11pt]">
                 <thead>
                    <tr className="bg-slate-50">
                       ...
                       <th className="border border-black p-2 text-center w-24">{activeCategory === 'extra' ? 'KEGIATAN' : 'TUKANG'}</th>
                       <th className="border border-black p-2 text-center w-24">{activeCategory === 'extra' ? 'JML HADIR' : 'JML HARI'}</th>
                       <th className="border border-black p-2 text-right w-32">TARIF / HR</th>
                       <th className="border border-black p-2 text-right w-36">JUMLAH (Rp)</th>
                       <th className="border border-black p-2 text-center w-28">TANDA TANGAN</th>
                    </tr>
                 </thead>
                 <tbody>
                    {currentStaffList.map((s, idx) => {
                      ...
                      return (
                        <tr key={s.id}>
                           ...
                           <td className="border border-black p-3 text-center text-[9pt]">{s.rank || '-'}</td>
                           <td className="border border-black p-3 text-center font-bold">{count}</td>
                           <td className="border border-black p-3 text-right">{rate.toLocaleString('id-ID')}</td>
                           <td className="border border-black p-3 text-right font-bold">{total.toLocaleString('id-ID')}</td>
                           <td className="border border-black p-3 text-left relative h-12">
                              <span className="text-[8pt] text-slate-300 absolute top-1 left-1">{idx + 1}.</span>
                           </td>
                        </tr>
                      );
                    })}
                    <tr className="font-bold bg-slate-50">
                       <td colSpan={5} className="border border-black p-3 text-right uppercase tracking-wider">TOTAL KESELURUHAN</td>
                       <td className="border border-black p-3 text-right font-black">
                          {currentStaffList.reduce((acc, s) => acc + calculateTotal(s.id), 0).toLocaleString('id-ID')}
                       </td>
              ...
              <div className="mt-12 flex justify-between px-10">
                 ...
                 <div className="text-center w-[200px]">
                    <p className="mb-20">Kediri, {format(new Date(year, month + 1, 0), 'dd MMMM yyyy', { locale: id })}<br/>Bendahara,</p>
                    <p className="font-bold underline uppercase">....................................</p>
                    <p className="text-[10pt]">NIP. ............................</p>
                 </div>
              </div>
              <div className="mt-auto pt-10 text-[8pt] text-slate-400 italic">Dicetak secara otomatis melalui Sistem ArsipDigital pada {format(new Date(), 'dd/MM/yyyy HH:mm')}</div>
           </div>
        </div>
      </div>
