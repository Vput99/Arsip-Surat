...
const SmartContentRenderer = ({ text }: { text: string }) => {
  ...
  const flushTable = () => {
    if (tableRows.length > 0) {
      renderedBlocks.push(
        <div key={`table-wrapper-${renderedBlocks.length}`} className="pl-8 mb-4 break-inside-avoid">
  ...
  lines.forEach((line, index) => {
    ...
    if (trimmed.startsWith('PASAL')) {
      flushTable();
      renderedBlocks.push(
        <div key={`pasal-${index}`} className="mt-6 mb-2 font-bold text-center break-after-avoid">
          {trimmed}
        </div>
      );
      return;
    ...
    if (colonIndex > 0 && colonIndex < 35 && trimmed.length > 0) {
         ...
         if (isSectionHeader) {
             flushTable();
             renderedBlocks.push(<p key={`p-${index}`} className="mb-2 text-justify font-serif break-inside-avoid">{line}</p>);
         } else {
      ...
      if (trimmed === '') {
         ...
         renderedBlocks.push(
           <p key={`p-${index}`} className={`mb-2 text-justify whitespace-pre-wrap break-inside-avoid font-serif ${isIndentedContext ? 'pl-8' : ''}`}>
             {line}
           </p>
         );
      }
    ...
    <div className="flex flex-col h-[calc(100vh-100px)] gap-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm print:hidden">
        <div>
          ...
          <p className="text-slate-500 text-sm">Pilih template dan cetak ke PDF.</p>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 h-full overflow-hidden">
        ...
        <div className="w-full lg:w-5/12 flex flex-col gap-4 overflow-y-auto pr-2 print:hidden">
          <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-4">
             ...
             {isMOU && (
               <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Nama Pihak Kedua</label>
             ...
             <div className="pt-2 border-t border-slate-100 space-y-3">
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Info Tanda Tangan (Sekolah)</label>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 flex-1 flex flex-col min-h-[400px]">
             <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Isi Surat (Editor)</label>
        ...
        <div className="w-full lg:w-7/12 bg-slate-200/50 rounded-2xl border border-slate-200 overflow-y-auto p-4 md:p-8 flex justify-center print:p-0 print:m-0 print:border-0 print:bg-white print:w-full print:block">
           <div className="letter-paper bg-white w-[215mm] min-h-[330mm] shadow-2xl p-[20mm] mx-auto relative print:shadow-none print:w-full print:m-0 print:p-[15mm] flex flex-col">
              {/* Header / Kop */}
              <div className="border-b-[4px] border-double border-black pb-4 mb-6 pt-2 grid grid-cols-[100px_1fr_100px] items-center">
                 <div className="flex justify-center items-center">
                   {config.logoDaerahUrl ? <img src={config.logoDaerahUrl} className="w-[22mm] h-auto object-contain max-h-[25mm]" alt="Logo Daerah"/> : <div className="w-[22mm]"></div>}
                 </div>
                 <div className="text-center w-full px-2">
                    ...
                    <p className="text-[9pt] font-serif leading-tight mt-1 text-black">{config.address}</p>
                    <p className="text-[9pt] font-serif leading-tight text-black">Email: {config.email}</p>
                 </div>
                 <div className="flex justify-center items-center">
                   {config.logoUrl ? <img src={config.logoUrl} className="w-[22mm] h-auto object-contain max-h-[25mm]" alt="Logo Sekolah"/> : <div className="w-[22mm]"></div>}
                 </div>
              </div>
              ...
              <div className="font-serif text-black leading-relaxed flex-1 flex flex-col">
                 <div className="text-center mb-6 break-inside-avoid">
                    ...
                    <p className="mt-0 font-bold text-[11pt]">NOMOR : {formData.refNumber}</p>
                 </div>
                 ...
                 {/* Tanda Tangan */}
                 <div className={`mt-10 break-inside-avoid ${isMOU ? 'grid grid-cols-2 gap-10' : 'flex justify-end'}`}>
                    {isMOU && (
                    ...
                    <div className="text-center font-serif text-[11pt] min-w-[200px]">
                       ...
                       {useQRCode ? (
                         <div className="h-24 flex justify-center items-center my-1">
                       ...
                       <p className="text-[10pt]">{formData.signerNip ? `NIP. ${formData.signerNip}` : ''}</p>
                    </div>
                 </div>

                 {/* BSrE Footer (Opsional) */}
                 {useQRCode && (
                   <div className="mt-12 border border-black p-2 flex items-center gap-3 w-full break-inside-avoid opacity-80">
                     <div className="flex items-center gap-2 shrink-0 border-r border-black pr-3">
                        ...
                        <div className="flex flex-col text-[6pt] font-bold leading-tight">
                          <span>Balai</span><span>Sertifikasi</span><span>Elektronik</span>
                        </div>
                     </div>
                     <div className="text-[6.5pt] font-serif text-black leading-tight italic">
                       Dokumen ini telah ditandatangani secara elektronik sesuai UU ITE No. 19 Tahun 2016.
                     </div>
                   </div>
                 )}
              </div>
      ...
      <style>{`
        @media print {
          ...
          /* Reset container agar memenuhi layar */
